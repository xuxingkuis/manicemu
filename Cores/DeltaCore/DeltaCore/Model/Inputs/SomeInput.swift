    ...
    public init(stringValue: String, intValue: Int?, type: InputType, isContinuous: Bool? = nil)
    {
        ...
        else
        {
            ...
            case .game(let gameType):
                guard let deltaCore = ManicEmu.core(for: gameType), let input = deltaCore.gameInputType.init(stringValue: self.stringValue) else { break }
...
public extension SomeInput
{
    ...
    init(from decoder: Decoder) throws
    {
        ...
        case .game(let gameType):
            guard let deltaCore = ManicEmu.core(for: gameType), let input = deltaCore.gameInputType.init(stringValue: stringValue) else {
