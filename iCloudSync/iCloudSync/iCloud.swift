...
open class iCloud: NSObject {
    ...
    func getAllFileRelativePaths(documentUrl: URL, filePath: String) -> [String] {
        var results: [String] = []
        if let enumerator = FileManager.default.enumerator(at: documentUrl.appendingPathComponent(filePath), includingPropertiesForKeys: [.isRegularFileKey], options: [], errorHandler: nil) {
            for case let fileURL as URL in enumerator {
                let resourceValues = try? fileURL.resourceValues(forKeys: [.isRegularFileKey])
                if resourceValues?.isRegularFile == true {
                    var path: String
                    if documentUrl.path.starts(with: "/private") {
                        path = fileURL.path.replacingOccurrences(of: documentUrl.path + "/", with: "")
                    } else {
                        path = fileURL.path.replacingOccurrences(of: "/private", with: "").replacingOccurrences(of: documentUrl.path + "/", with: "")
                    }
                    results.append(path)
                }
            }
    ...
    open func uploadLocalOfflineDocuments(repeatingHandler: ((String?, Error?) -> Void)? = nil, completion: (() -> Void)? = nil) {
        ...
        guard
            ...
            var localFiles: [String] = try? self.fileManager.contentsOfDirectory(atPath: localDocuments.path)
            else { return }
        
        var newLocalFiles = [String]()
        for localFile in localFiles {
            let localFileUrl = localDocuments.appendingPathComponent(localFile)
            if isDirectory(at: localFileUrl) {
                //文件夹
                newLocalFiles.append(contentsOf: getAllFileRelativePaths(documentUrl: localDocuments, filePath: localFile))
            } else {
                //文件
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            for item in localFiles {
                ...
                guard (self.previousQueryResults.map{ $0.name }).contains(item) else {
                    ...
                    do {
                        let cloudParentUrl = cloudURL.deletingLastPathComponent()
                        if !self.fileManager.fileExists(atPath: cloudParentUrl.path) {
                            try self.fileManager.createDirectory(at: cloudParentUrl, withIntermediateDirectories: true)
                        }
...
//                    }
    ...
    open var listCloudFileNames: [String]? {
        get {
            if let urls = listCloudFiles, let cloudUrl = self.cloudDocumentsURL {
                var results = [String]()
                for url in urls {
                    if isDirectory(at: url) {
                        //文件夹
                        results.append(contentsOf: getAllFileRelativePaths(documentUrl: cloudUrl, filePath: url.lastPathComponent))
                    } else {
                        //文件
                        results.append(url.path.replacingOccurrences(of: cloudUrl.path + "/", with: ""))
                    }
                }
                if results.count > 0 {
                    return results
