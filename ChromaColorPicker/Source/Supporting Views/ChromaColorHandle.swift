...
//        return path.cgPath
        ...
        return UIBezierPath(ovalIn: circleRect).cgPath
    }
    ...
    internal func layoutAccessoryViewIfNeeded() {
        if let accessoryLayer = accessoryView?.layer {
            let contentWidth = bounds.width - borderWidth * 2
            let width = contentWidth*0.8
            let size = CGSize(width: width - (accessoryViewEdgeInsets.left + accessoryViewEdgeInsets.right),
                              height: width - (accessoryViewEdgeInsets.top + accessoryViewEdgeInsets.bottom) - 4)
            accessoryLayer.frame = CGRect(origin: CGPoint(x: (borderWidth / 2) + (contentWidth - width)/2 + accessoryViewEdgeInsets.left, y: (borderWidth / 2) + (contentWidth - width)/2 + accessoryViewEdgeInsets.top + 5), size: size)
